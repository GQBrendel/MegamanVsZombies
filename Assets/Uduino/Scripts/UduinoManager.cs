...
#region Enums
    ...
    public class UduinoManager : MonoBehaviour {
        ...
        void Awake()
        {
            ...
            if (autoDiscover)
                StartCoroutine("DelayedDiscover");
        ...
        IEnumerator DelayedDiscover()
        {
            yield return new WaitForSeconds(delayBeforeDiscover);
            DiscoverPorts();
        }
        ...
        public void pinMode(UduinoDevice target, int pin, PinMode mode)
        {
            ...
            foreach (Pin pinTarget in pins)
            {
                ...
                {
                    Log.Debug("Override pinMode for the pin <color=#4CAF50>" + pin + "</color> on the arduino <color=#ff3355>" + target.name + "</color> from <color=#2e7d32>" + pinTarget.pinMode + "</color> to <color=#2e7d32>" + mode + "</color>.");
        ...
        */
        ...
        {
            return digitalRead(null, (int)pin);
        }

        public int digitalRead(UduinoDevice target, AnalogPin pin)
        {
            return digitalRead(target, (int)pin);
        }
        ...
        {
            return analogRead(target, (int)pin, bundle);
        }
        ...
        {
            return analogRead(null, (int)pin, bundle);
        }
        ...
        public void CloseDevice(UduinoDevice device)
        {
            try {
                ...
                OnBoardConnectedEvent.Invoke(device);
...
#region Version
    public static class UduinoVersion
    {
        ...
        static int minor = 1;
        static int patch = 0;
        static string update = "March 2018";
