    ...
    public class UduinoDevice_DesktopSerial : UduinoDevice
    {
        ...
        public override bool ReadFromArduinoLoop(bool forceReading = false)
        {
           ...
           // serial.DiscardInBuffer(); // ! Creates bug in MacOS
            ...
            {
                if (e.InnerException.GetType() == typeof(System.IO.IOException))
                {
                ...
                if (UduinoManager.Instance.autoReconnect)
                    UduinoManager.Instance.DiscoverPorts(); // TODO : Test with multiple board (it will detec all again). Should connect only to this one. 
            }
            return false;
        }
        #endregion
